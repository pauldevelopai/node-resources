 ...
 * data agree.
 */
export function tenantStore(newsroomId, poolFn = requirePool) {
  if (!newsroomId) throw new Error('tenantStore needs a newsroom id');
  // The pool arrives as a function, not a value, so it stays lazy — and so a
  // caller can inject one. That injection seam is the engine's own convention
  // (it takes its pg pool and chat function the same way), and it is what lets
  // the tenancy tests assert which id every query is actually bound to without
