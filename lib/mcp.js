...
async function toolGetProfile(newsroomId, args, host) {
  const criteria = await getCriteria(orgStore(host, newsroomId));
...
async function toolUpdateProfile(newsroomId, args, host) {
  ...
  const saved = await saveCriteria(orgStore(host, newsroomId), patch);
  ...
  const rpc = async (req, res) => {
    ...
    if (method === 'tools/call') {
      ...
      try {
        ...
        // A host scoped to the key's tenant, for the shared cross-node profile.
        // The org's OWN state (criteria, docs) no longer comes from host.store
        // — handlers build it with orgStore(host, newsroomId), so MCP and the
        // web routes read the one place. They previously did not: the routes
        // got host.store keyed on the user, while the fake user id below made
        // MCP read it keyed on the newsroom, so criteria set in the UI were
        // invisible over MCP and vice versa. The fake id stays only because
        // hostFor needs a user shape to build a host at all.
