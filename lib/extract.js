...
// A money figure the scorer can trust, or nothing.
//
// The model is asked for digits (see the prompt), but a model can still return
// "1.5 million" or "R50 000". This REFUSES anything that isn't already a clean
// number rather than guessing at it: a wrong magnitude scores a great grant as
// zero and routes it red, where a null merely scores it "not stated" and a
// person still sees it. An unparseable amount is a gap to be honest about, not
// a number to invent.
function toAmount(v) {
  if (v == null || v === '') return null;
  if (typeof v === 'number') return Number.isFinite(v) && v >= 0 ? v : null;
  const s = String(v).trim().replace(/[\s,]/g, '');   // "50 000" / "50,000" are unambiguous
  if (!/^\d+(\.\d+)?$/.test(s)) return null;          // anything else (words, ranges, symbols) -> unknown
...
const EXTRACT_SYSTEM =
  ...
  `  "amount_min": number|null,                 // the SAME figure as digits only, no separators, no words: "ZAR 1.5 million" -> 1500000, "$50k" -> 50000\n` +
  `  "amount_max": number|null,                 // a stated RANGE fills both ("USD 50,000-100,000" -> 50000 and 100000); ONE stated figure sets both to it; "up to X" sets max only\n` +
  `  "amount_currency": string|null,            // ISO code if identifiable: "USD"|"ZAR"|"EUR"|"GBP"... null if the text gives no currency\n` +
...
export const extractCallFields = makeFieldExtractor({
  ...
  normalise: (p) => ({
    ...
    amount_min: toAmount(p.amount_min),
    amount_max: toAmount(p.amount_max),
    amount_currency: p.amount_currency ? String(p.amount_currency).trim().toUpperCase().slice(0, 8) : null,
    // What the grant_size rule scores on. Derived, not invented: it PICKS
    // between two figures the funder stated, preferring the ceiling because
    // "how much could we actually get" is the fundraiser's question. Null when
    // neither was stated, so the rule reports "not stated" instead of zero.
    // Kept out of the column map on purpose — it belongs in `extracted`, where
    // it is visibly derived rather than looking like something the call said.
    amount_for_scoring: toAmount(p.amount_max) ?? toAmount(p.amount_min),
