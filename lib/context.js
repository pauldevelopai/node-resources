...
export const emptyCriteria = () => ({
  ...
  exclusions: '',      // funders / conditions the org won't take, in prose (grounds the AI)
  exclusion_terms: [], // the same refusal as discrete terms — becomes a HARD scoring rule
  updated_at: null,
});

// These take the ORG's store explicitly (see lib/store.js) rather than reaching
// for host.store themselves. The criteria card, the documents, the chats and the
// proposals all belong to the organisation, and host.store is keyed on the
// signed-in USER — so taking the store as an argument is what keeps a client's
// colleagues looking at the same thing. Callers get it from orgStore(host, id).

export async function getCriteria(store) {
  const saved = await store.get('criteria', CRITERIA_KEY);
  return { ...emptyCriteria(), ...(saved || {}) };
}

export async function saveCriteria(store, patch) {
  const merged = { ...(await getCriteria(store)), ...patch, updated_at: new Date().toISOString() };
  await store.put('criteria', CRITERIA_KEY, merged);
  return merged;
}

export async function listDocs(store) {
  const docs = (await store.list('docs')).map((d) => ({ key: d.key, ...d.value }));
 ...
 *
 * Takes `host` for the shared cross-node profile AND the org store for this
 * Node's own criteria and documents.
 */
export async function orgContext(host, store, { withDocs = false, docLimit = 6 } = {}) {
  const parts = [];
  ...
  const c = await getCriteria(store);
  ...
  const critLines = [
    ...
    list(c.exclusion_terms) && `Funders and terms we rule out entirely: ${list(c.exclusion_terms)}`,
  ].filter(Boolean);
  if (critLines.length) parts.push('SEARCH CRITERIA (set by the org, editable any time):\n' + critLines.join('\n'));

  if (withDocs) {
    const docs = (await listDocs(store)).slice(0, docLimit);
    if (docs.length) {
      parts.push(
