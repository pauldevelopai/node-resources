...
//
// Storage split — BOTH halves keyed on the newsroom, which took a fix:
...
//   resources.tenant_store via orgStore() — prose criteria card, org docs,
//     chats, proposal drafts. NOT host.store: the runtime keys that on the
//     signed-in USER, which split a client's team down the middle (see
//     lib/store.js). orgStore falls back to host.store only on a DB-less local
//     install, where there is one person anyway.
...
import { tenantOf, pipelineFor, refreshCriteriaFromForm, saveRules, describeCriteria, SCHEMA } from './engine.js';
...
export function mountAppRoutes(app, getHost) {
  ...
  // The ORG's store for this request. Every route that touches the criteria
  // card, documents, chats or proposals goes through here, so the newsroom
  // keying is decided in exactly one place.
  const storeFor = async (req, host) => orgStore(host, (await tenantOf(req)).id);
  ...
  // ─── Attributing a find to the place it came from ───────────────────
  // The org's source list is a promise: each source keeps a running count of
  // what it has produced. That only means anything if a find is credited to the
  // source it actually came from, so the scan matches every candidate URL back
  // to the org's own listed sources and stamps the item with that source id.
  // Unmatched finds go to 'Web scan' — honest: the web at large produced them.
  // Tolerant on purpose: staff type "fundsforngos.org" as often as a full URL,
  // and a source that silently never matches is worse than no source at all.
  const hostOf = (url) => {
  ...
  /** Build a URL → sourceId matcher from the tenant's listed sources. */
  function sourceMatcher(sources) {
    const hosts = sources
      .map((s) => ({ id: s.id, host: hostOf(s.location) }))
      .filter((s) => s.host)
      // Longest host first: a specific source (grants.example.org) beats a
      // broader one (example.org) when both are listed.
      .sort((a, b) => b.host.length - a.host.length);
    return (url) => {
      const h = hostOf(url);
      if (!h) return null;
      const hit = hosts.find((s) => h === s.host || h.endsWith('.' + s.host));
  ...
  app.get('/api/overview', wrap(async (req, host) => {
    const store = await storeFor(req, host);
    const [criteria, docs] = await Promise.all([getCriteria(store), listDocs(store)]);
    const profile = host.profile ? await host.profile.get().catch(() => null) : null;
    const dbReady = !!getPool();
    let opportunities = [], sources = [], rules = null;
    if (dbReady) {
      ...
      sources = rows;
      rules = await describeCriteria(requirePool(), tenant.id);
    }
    return {
      ok: true, dbReady, criteria, profile, opportunities, sources, rules,
  ...
  app.post('/api/criteria', wrap(async (req, host) => {
    ...
    // Grant size. Blank is a real answer meaning "no range" — it clears the
    // bounds and stands the rule down, so it must survive as null rather than
    // becoming 0, which would read as "we'll take grants of nothing".
    const asAmount = (v) => {
      if (v == null || String(v).trim() === '') return null;
      const n = Number(String(v).replace(/[\s,]/g, ''));
      if (!Number.isFinite(n) || n < 0) {
        throw Object.assign(new Error('Grant sizes need to be plain numbers — "50000", not "50k".'), { status: 400 });
      }
      return n;
    };
    const criteria = await saveCriteria(await storeFor(req, host), {
      ...
      exclusion_terms: asList(b.exclusion_terms),
      amount_min: asAmount(b.amount_min),
      amount_max: asAmount(b.amount_max),
    });
    ...
    // Hand back the regenerated rules so the panel shows the effect immediately
    // — "a change takes effect the moment it is saved" should be visible.
    return { ok: true, criteria, scoringRefreshed, rules };
  }));

  // ─── The rules panel: see exactly how an opportunity gets its band ──
  // The org is promised it can see AND change the rules. The criteria card
  // decides what each rule looks for; this decides how much each one counts
  // and where the green/red lines fall.
  app.get('/api/rules', wrap(async (req, _host) => {
    if (!getPool()) return { ok: true, rules: null, dbReady: false };
    const tenant = await tenantOf(req);
    return { ok: true, dbReady: true, rules: await describeCriteria(requirePool(), tenant.id) };
  }));

  app.post('/api/rules', wrap(async (req, host) => {
    const tenant = await tenantOf(req);
    const b = req.body || {};
    const version = await saveRules(requirePool(), tenant.id, {
      weights: Array.isArray(b.weights) ? b.weights : [],
      thresholds: b.thresholds || {},
    });
    await host.log.run({ op: 'rules_save', details: { version } });
    return { ok: true, version, rules: await describeCriteria(requirePool(), tenant.id) };
  }));
  ...
  app.post('/api/scan', wrap(async (req, host) => {
    const tenant = await tenantOf(req);
    const context = await orgContext(host, await storeFor(req, host));
    ...
    const { rows: srcRows } = await pool.query(
      `SELECT id, name, location FROM ${SCHEMA}.sources WHERE newsroom_id = $1 AND active = true AND location IS NOT NULL`, [tenant.id]);
    ...
    const webScanId = await ensureNamedSource(tenant.id, 'Web scan', 'search');
    const matchSource = sourceMatcher(srcRows);
    ...
    const items = found.filter((f) => f && f.text).map((f) => ({
      ...
      // Credited to the org's own source when the URL came from there.
      sourceId: matchSource(f.url) || webScanId,
    }));
    // Cross-source dedup. raw_items dedups on (source_id, external_id), so a
    // call first credited to 'Web scan' and later matched to a source the org
    // added since would come back as a SECOND opportunity. The URL is the
    // identity a person recognises — drop what we already hold for this tenant
    // and report it as already-known rather than showing the same call twice.
    const urls = [...new Set(items.map((i) => i.url).filter(Boolean))];
    let held = new Set();
    if (urls.length) {
      const { rows } = await pool.query(
        `SELECT url FROM ${SCHEMA}.funding_calls WHERE newsroom_id = $1 AND url = ANY($2::text[])`,
        [tenant.id, urls]);
      held = new Set(rows.map((r) => r.url));
    }
    const fresh = items.filter((i) => !(i.url && held.has(i.url)));
    const alreadyHeld = items.length - fresh.length;

    // sourceId: null at run level — this run spans several sources, and each
    // item carries its own. The engine honours the per-item id (and so the
    // per-source counters move for the places the org actually listed).
    const out = await pipeline.runPipeline({ newsroomId: tenant.id, sourceId: null, items: fresh });
    // Honest counts: what we looked at includes what we recognised and skipped.
    const digest = {
      ...out.digest,
      seen: (out.digest.seen || 0) + alreadyHeld,
      duplicate: (out.digest.duplicate || 0) + alreadyHeld,
    };

    // What each of the org's OWN sources produced this run — so the scan can say
    // it, not just leave it to be inferred from the sources card.
    const nameById = new Map(srcRows.map((s) => [String(s.id), s.name]));
    const attribution = Object.entries(out.perSource || {})
      .filter(([sid]) => nameById.has(String(sid)))
      .map(([sid, c]) => ({ source: nameById.get(String(sid)), seen: c.seen, new: c.new }))
      .sort((a, b) => b.new - a.new || b.seen - a.seen);
    ...
    await host.log.run({ op: 'scan', details: { ...digest, corpusWritten, ownSources: attribution.length } });
    return { ok: true, digest, corpusWritten, attribution, opportunities: await listCalls(tenant.id) };
  }));
  ...
  app.post('/api/opportunity', wrap(async (req, host) => {
    ...
    const store = orgStore(host, tenant.id);
    const chat = (await store.get('chats', id)) || { messages: [] };
    const proposal = (await store.get('proposals', id)) || null;
  ...
  app.post('/api/opportunities/delete', wrap(async (req, host) => {
    ...
    const store = orgStore(host, tenant.id);
    await store.delete('chats', id);
    await store.delete('proposals', id);
    return { ok: true };
  }));
  ...
  app.post('/api/chat', wrap(async (req, host) => {
    ...
    const store = orgStore(host, tenant.id);
    const thread = (await store.get('chats', id)) || { messages: [] };
    const history = thread.messages.slice(-12).map((m) => `${m.role === 'user' ? 'THEM' : 'YOU'}: ${m.content}`).join('\n\n');
    const context = await orgContext(host, store, { withDocs: true, docLimit: 3 });
    ...
    await store.put('chats', id, thread);
    await host.log.run({ op: 'chat', details: { id } });
    return { ok: true, reply: text, chat: thread };
  }));

  // ─── Internal documents (ground the proposal writing) ──────────────
  app.get('/api/docs', wrap(async (req, host) => ({
    ok: true,
    docs: (await listDocs(await storeFor(req, host))).map(({ text, ...d }) => ({ ...d, chars: String(text || '').length })),
  })));
  ...
  app.post('/api/docs', wrap(async (req, host) => {
    ...
    await (await storeFor(req, host)).put('docs', key, { title, kind, text: text.slice(0, 60000), added_at: new Date().toISOString() });
    await host.log.run({ op: 'doc_add', bytes: text.length });
    return { ok: true };
  }));

  app.post('/api/docs/delete', wrap(async (req, host) => {
    await (await storeFor(req, host)).delete('docs', String(req.body?.key || ''));
    return { ok: true };
  }));
  ...
  app.post('/api/proposal', wrap(async (req, host) => {
    ...
    const store = orgStore(host, tenant.id);
    const prev = (await store.get('proposals', id)) || null;
    const context = await orgContext(host, store, { withDocs: true, docLimit: 6 });
    const fp = opp.funder_profile;
    ...
    await store.put('proposals', id, proposal);
